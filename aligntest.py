...
vidconfig = {
    ...
    "linespacing": 20,  # In percentage of font height
}
...
def make_video():
    ...
    background.save("overlay.png", transparency = 0, optimize = 1)

    #return  # Skip video creation

    img_clip = (ImageClip("overlay.png")
                .set_position(('center', 'bottom'))
                .set_duration(17)
               )

    vid_name = prefix + "_" + "_".join([str(pid) for pid in pids])
    result = CompositeVideoClip([basevid, img_clip])  # Overlay text on video
    print "Saving final video file.."
    result.write_videofile(vid_dest_dir + "/" + vid_name + ".mp4", fps=25)  # Many options...
